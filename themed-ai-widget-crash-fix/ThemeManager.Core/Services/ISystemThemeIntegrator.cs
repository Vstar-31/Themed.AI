...
public interface ISystemThemeIntegrator
{
    ...
    /// <summary>True while Windows' own Battery Saver mode is turned on. Used by
    /// <see cref="ThemeAutomationService"/> to switch to a low-power-friendly theme; returns false
    /// (never throws) on desktops with no battery or if the underlying API is unavailable.</summary>
    Task<bool> IsBatterySaverActiveAsync();
}
