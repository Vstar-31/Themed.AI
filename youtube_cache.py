...
Usage in main.py (/api/vibe/analyze):
    from core.youtube_cache import resolve_video_id

    # Cache-checked search-and-cache in one call, so callers of /api/vibe/
    # analyze don't need a second request to get a playable video_id.
"""
...
async def resolve_video_id(title: str, artist: str) -> Optional[str]:
    """
    Resolves a real, playable YouTube video_id for (title, artist) — the piece
    the desktop client (Themed.AI) was missing. It was calling the IFrame
    Player API's loadPlaylist({listType: 'search', list: query}) directly from
    the player itself, which YouTube deprecated on 15 Nov 2020: calls with
    listType='search' now return a 4xx and the player never loads anything
    (see https://developers.google.com/youtube/iframe_api_reference). A
    videoId has to be resolved server-side first, same as our own frontend
    already does via GET /api/services/youtube/search — this gives /api/vibe/
    analyze callers that same videoId directly, one request instead of two.

    Checks the shared cache (yt_cache_get) before spending quota, same as
    /api/services/youtube/search. Returns None on any failure — no key
    configured, quota exceeded, no results, network error — rather than
    raising, so one track's YouTube lookup failing doesn't take down the rest
    of an analyze response; mirrors how a failed iTunes preview lookup
    degrades to preview_url: None instead of an error.
    """
    ...
    try:
        ...
        if "error" in data:
            if data["error"].get("code") == 403:
                logger.warning(f"[YTCache] Quota exceeded resolving video_id for '{title}'")
            return None
        ...
        logger.debug(f"[YTCache] resolve_video_id failed for '{title}': {e}")
        return None
