...
public sealed partial class SkinHostWindow : Window
{
    ...
    // Cached once instead of the previous App.LoggerFactory.CreateLogger<SkinHostWindow>() calls
    // scattered through this file (each of which allocated a brand-new logger instance on every
    // single failure) — same underlying category name, just one instance for the window's
    // lifetime. This is also the ONLY logger this class had access to; the interop calls it makes
    // (HideFromTaskbarAndAltTab, SetClickThrough, DesktopLayerInterop.TryAttach/Detach) all accept
    // an optional ILogger but were never given one, so none of their failure paths ever reached a
    // log sink from here even though the plumbing already existed.
    private readonly ILogger _logger = App.LoggerFactory.CreateLogger<SkinHostWindow>();
    ...
    private void EnableTransparency()
    {
        ...
        {
            _logger.LogWarning(ex, "True transparency failed to apply; widget will render on a solid card instead");
        }
    }
    ...
    public void PrepareForClose()
    {
        try
        {
            ...
            {
                DesktopLayerInterop.Detach(_hwnd, _logger);
            }
        ...
        {
            // Previously a bare `catch { }` — this runs during window teardown, so a failure here
            // can't be surfaced to the user either way, but it's still worth a trace: if widgets
            // ever start leaking native resources on close, this is the first place to look.
            _logger.LogWarning(ex, "PrepareForClose: cleanup threw for skin \"{Skin}\"", _viewModel.Definition.Name);
        }
    }
    ...
    public void ApplyClickThrough(bool enabled) =>
        SkinWindowInterop.SetClickThrough(_hwnd, enabled, _logger);

    public Task<bool> ApplyDesktopLayerAsync(bool enabled)
    {
        return Task.Run(() =>
        {
            if (enabled) return DesktopLayerInterop.TryAttach(_hwnd, _logger);
            DesktopLayerInterop.Detach(_hwnd, _logger);
            return true;
        });
    }
            ...
            element.PointerPressed += (s, e) =>
            {
                ...
                try
                {
                    ...
                    if (url.StartsWith(vibeFinderPreviewPrefix, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (vibeMeasure != null)
                        {
                            _logger.LogDebug("Skin \"{Skin}\": preview-play click -> VibeFinderPreviewPlayer.Play({Url})",
                                _viewModel.Definition.Name, vibeMeasure.CurrentPreviewUrl ?? "(null)");
                            VibeFinderPreviewPlayer.Play(vibeMeasure.CurrentPreviewUrl);
                        }
                        else
                        {
                            // Previously a complete silent no-op with no fallback and nothing logged —
                            // this specific URL scheme only exists on a VibeFinderMeasure's own meter,
                            // so a widget without one clicking it is a skins.json authoring mistake, not
                            // a runtime state to recover from. Worth a Warning so it's visible either way.
                            _logger.LogWarning("Skin \"{Skin}\": clicked a vibefinder/preview URL but this widget has no VibeFinderMeasure", _viewModel.Definition.Name);
                        }
                    }
                    else if (url.StartsWith(mediaPrefix, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        // Falls through here when there's no VibeFinderMeasure on this widget at all —
                        // treat it as a plain OS "Now Playing" command instead (a non-Vibe media meter).
                        _logger.LogDebug("Skin \"{Skin}\": no VibeFinderMeasure on this widget — forwarding \"{Command}\" to MediaMeasure (OS Now Playing)",
                            _viewModel.Definition.Name, command);
                ...
                {
                    _logger.LogWarning(ex, "Skin \"{Skin}\": failed to handle action URL {Url}", _viewModel.Definition.Name, url);
                }
            };
