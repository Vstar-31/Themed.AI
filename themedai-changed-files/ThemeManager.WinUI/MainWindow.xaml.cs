...
{
    // Same App.LoggerFactory.CreateLogger<MainWindow>() category the one existing call below
    // already used, just cached instead of allocated fresh on every prewarm failure.
    ...
    public async void EnsureVibeFinderPrewarm()
    {
        ...
        {
            _logger.LogDebug("EnsureVibeFinderPrewarm: already started — ignored");
            return;
        }
        ...
        {
            _logger.LogDebug("EnsureVibeFinderPrewarm: no saved credentials — skipping");
            return;
        }

        if (!NetworkStatus.IsInternetAvailable())
        {
            _logger.LogInformation("EnsureVibeFinderPrewarm: no internet — deferring until NetworkStatusChanged fires");
            SubscribeVibeFinderNetworkRetry();
            return;
        }

        _vibeFinderPrewarmStarted = true;
        _logger.LogInformation("EnsureVibeFinderPrewarm: starting for user \"{User}\"", user);
        ...
        VibeFinderPrewarmWebView.CoreWebView2.NavigationCompleted += async (sender, args) =>
        {
            if (!args.IsSuccess)
            {
                _logger.LogWarning("EnsureVibeFinderPrewarm: navigation to {Uri} failed (WebErrorStatus={Status})", sender.Source, args.WebErrorStatus);
                return;
            }
            ...
            _logger.LogDebug("EnsureVibeFinderPrewarm: navigation completed for {Uri}", uri);

            try { await sender.ExecuteScriptAsync(VibeFinderAuth.SkipTutorialScript); }
            catch (Exception ex) { _logger.LogDebug(ex, "EnsureVibeFinderPrewarm: SkipTutorialScript injection failed — WebView2 may have been torn down"); }
            ...
            try
            {
                ...
                _logger.LogDebug("EnsureVibeFinderPrewarm: auto-login script injected for user \"{User}\"", u);
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "EnsureVibeFinderPrewarm: auto-login script injection failed for user \"{User}\" — WebView2 may have been torn down", u);
            }
        };
    ...
    {
        _logger.LogDebug("ResetVibeFinderPrewarm: clearing prewarm-started flag and retrying");
    ...
    private async System.Threading.Tasks.Task HandleVibeFinderLoginResultAsync(string json)
    {
        ...
        if (success)
        {
            _logger.LogInformation("HandleVibeFinderLoginResultAsync: prewarm login succeeded");
            return;
        }

        _logger.LogWarning("HandleVibeFinderLoginResultAsync: prewarm login failed, reason={Reason}", reason ?? "(unspecified)");
    ...
    private async System.Threading.Tasks.Task ShowVibeFinderIssueDialogAsync(string title, string message)
    {
        ...
        {
            _logger.LogDebug(ex, "ShowVibeFinderIssueDialogAsync(\"{Title}\") failed — e.g. window closing mid-dialog", title);
        }
        finally
        {
