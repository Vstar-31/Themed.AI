...
public static class MeasureFactory
{
    ...
    public static IMeasure Create(MeasureDefinition definition, ILogger? logger = null, IActiveThemeProvider? activeThemeProvider = null)
    {
        ...
        if (measure is UnknownMeasure)
        {
            ...
            // (e.g. hand-edited, or written by a version of the app that added a type this one
            // doesn't know about) resolved here with zero trace, and the resulting widget just
            // showed "—" forever with nothing suggesting why.
            logger?.LogWarning("MeasureFactory: MeasureType {Type} for measure \"{Name}\" did not match any known measure — falling back to UnknownMeasure (always shows \"—\")", definition.Type, definition.Name);
        }
        return measure;
    }
