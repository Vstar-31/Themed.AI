...
public sealed class VibeFinderMeasure : IMeasure
{
    ...
    // Used only to avoid re-logging the same Text every single 1-second tick (Refresh() is
    // called once per second per open widget by SkinManagerService) — logging is gated on
    // this actually changing, not on how often Refresh() itself is called.
    ...
    public void Refresh()
    {
        ...
        {
            if (!string.Equals(Text, "Config Err", StringComparison.Ordinal))
                _logger.LogWarning("VibeFinderMeasure[{MeasureName}]: Target is missing username|password|phrase — showing Config Err", Name);
            Text = "Config Err";
            return;
        }
        ...
        {
            _logger.LogDebug("VibeFinderMeasure[{MeasureName}]: cache stale (or empty), kicking off a background FetchAsync", Name);
            Task.Run(() => FetchAsync(entry, parts[0], parts[1], vibeText));
        }

        // Snapshot Tracks + CurrentIndex together, under the same lock FetchAsync/SkipNext/
        // SkipPrevious use to mutate them. Previously this read `entry.Tracks[entry.CurrentIndex]`
        // directly with NO lock at all, while those three methods reassign both fields together
        // under `entry.Lock` — a background FetchAsync landing a new (possibly shorter) track
        // list in between this thread reading the old Tracks and the old CurrentIndex could throw
        // ArgumentOutOfRangeException right here. That exception used to propagate all the way up
        // to SkinManagerService.TickAll's generic "A widget failed to refresh its measures this
        // tick" catch — which aborts the REST of that widget's measures for the tick too (see
        // SkinHostViewModel.RefreshMeasures) — with nothing in the log identifying which measure,
        // target, or index was involved. This is the most likely explanation for "themed.ai
        // started bugging after a few song switches": rapid Next/Previous clicks plus a
        // same-moment background refetch racing on these two fields.
        System.Collections.Generic.List<(string Title, string Artist, string Mood, string? SpotifyUrl, string? AppleUrl, string? CoverArtUrl, string? PreviewUrl, string? VideoId)> tracksSnapshot;
        int indexSnapshot;
        lock (entry.Lock)
        {
            tracksSnapshot = entry.Tracks;
            indexSnapshot = entry.CurrentIndex;
        }

        if (tracksSnapshot.Count > 0)
        {
            if (indexSnapshot < 0 || indexSnapshot >= tracksSnapshot.Count)
            {
                // Defensive clamp — should be unreachable now that Refresh() and the mutators
                // agree on taking the lock together, but a clamp-and-log beats a crash if some
                // future edit reintroduces the race, and the log line pinpoints exactly that.
                _logger.LogWarning("VibeFinderMeasure[{MeasureName}]: CurrentIndex {Index} out of range for {Count} tracks — clamping to 0", Name, indexSnapshot, tracksSnapshot.Count);
                indexSnapshot = 0;
            }
            var data = tracksSnapshot[indexSnapshot];
        ...
        else if (vibeText is null)
        {
            ...
            if (!string.Equals(Text, "Config Err", StringComparison.Ordinal))
                _logger.LogWarning("VibeFinderMeasure[{MeasureName}]: Target uses \"$theme\" but no IActiveThemeProvider is wired up — showing Config Err", Name);
            Text = "Config Err";
        }
    ...
    /// <summary>Logs at Debug only when the computed Text actually changed since the last
    /// Refresh() — Refresh() itself runs once a second per open widget, and logging every tick
    /// unconditionally would bury the transitions that actually matter (a track/state change)
    /// under routine noise.</summary>
    ...
    private string? ResolveVibeText(string rawPhrase)
    {
        ...
        if (theme is null)
        {
            _logger.LogDebug("VibeFinderMeasure[{MeasureName}]: target is \"$theme\" but _activeThemeProvider is null", Name);
            return null;
        }
        return ThemeVibeText.Describe(theme);
    }
    ...
    private async Task FetchAsync(ResultEntry entry, string username, string password, string vibeText)
    {
        ...
        _logger.LogInformation("VibeFinderMeasure[{MeasureName}]: fetching recommendation for user \"{User}\", phrase \"{Phrase}\"", Name, username, vibeText);
        ...
        try
        {
            ...
            {
                _logger.LogDebug("VibeFinderMeasure[{MeasureName}]: cached token rejected (401) — retrying with a fresh login", Name);
            ...
            int totalReturned = root.TryGetProperty("tracks", out var countEl) && countEl.ValueKind == JsonValueKind.Array
                ? countEl.GetArrayLength() : 0;
            if (newTracks.Count == 0)
            {
                // This is the exact state that leaves the widget's play button permanently dead
                // (PreviewUrl null, nothing for VibeFinderPreviewPlayer to play) — worth a
                // Warning, not Debug, since from the outside it looks identical to "click did
                // nothing" with no other symptom.
                _logger.LogWarning(
                    "VibeFinderMeasure[{MeasureName}]: backend returned {Total} track(s) but all were filtered out (missing cover_art) — falling back to \"No match\" placeholder with no playable preview",
                    Name, totalReturned);
            ...
            else
            {
                _logger.LogInformation(
                    "VibeFinderMeasure[{MeasureName}]: fetched {Kept}/{Total} track(s) in {ElapsedMs}ms, dominant vibe \"{Mood}\"",
                    Name, newTracks.Count, totalReturned, sw.ElapsedMilliseconds, mood);
            }
    ...
    private async Task<string?> GetTokenAsync(string username, string password, bool forceRefresh = false)
    {
        ...
        try
        {
            ...
            if (!resp.IsSuccessStatusCode)
            {
                _logger.LogWarning("VibeFinderAI /auth/token returned {Status} for user \"{User}\"", resp.StatusCode, username);
                return null;
            }
            ...
            if (token is null)
                _logger.LogWarning("VibeFinderAI /auth/token returned 2xx but no access_token for user \"{User}\"", username);
            else
                _logger.LogDebug("VibeFinderAI: {Action} for user \"{User}\"", forceRefresh ? "refreshed token" : "obtained new token", username);
    ...
    {
        _logger.LogDebug("VibeFinderMeasure[{MeasureName}]: SkipNext (webEmbedActive={WebActive})", Name, VibeFinderWebState.IsActive);
        if (VibeFinderWebState.IsActive)
        {
        ...
        lock (entry.Lock)
        {
            if (entry.Tracks.Count > 0)
                ...
                _logger.LogDebug("VibeFinderMeasure[{MeasureName}]: SkipNext ignored — no cached tracks yet", Name);
        }
        Refresh();
    }
    ...
    {
        _logger.LogDebug("VibeFinderMeasure[{MeasureName}]: SkipPrevious (webEmbedActive={WebActive})", Name, VibeFinderWebState.IsActive);
        if (VibeFinderWebState.IsActive)
        {
        ...
        lock (entry.Lock)
        {
            ...
            {
                _logger.LogDebug("VibeFinderMeasure[{MeasureName}]: SkipPrevious ignored — no cached tracks yet", Name);
            }
        }
        Refresh();
    }
